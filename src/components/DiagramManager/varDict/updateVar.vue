    ...
    <div>
      ...
      >
        ...
        <el-form-item label="可编辑节点" prop="allowEditNodeId">
          <el-select v-model="allowEditNodeIdList" placeholder="可编辑节点" clearable multiple>
            <el-option
        ...
        <el-form-item label="必须填写节点" prop="requiredNodeId">
          <el-select v-model="requiredNodeIdList" placeholder="必须填写节点" clearable multiple>
            <el-option
              v-for="(item) in nodeList"
              :key="item.value"
              :label="item.label"
        ...
        <el-form-item label="隐藏当前字段节点" prop="hiddenNodeId">
          <el-select v-model="hiddenNodeIdList" placeholder="隐藏当前字段节点" clearable multiple>
            <el-option
              v-for="(item) in nodeList"
              :key="item.value"
              :label="item.label"
...
import { updateVarDict } from '@/api/varDict'
...
export default {
  ...
  watch: {
    watch: {
      ...
      processVar: {
        handler(newValue, oldValue) {
          this.form = deepClone(this.newValue)
          this.requiredNodeIdList = this.form.requiredNodeId.split(',') || []
          this.allowEditNodeIdList = this.form.allowEditNodeId.split(',') || []
          this.hiddenNodeIdList = this.form.hiddenNodeId.split(',') || []
        },
        deep: true
      }
    }
  ...
  mounted() {
    this.loading = false
    this.form = deepClone(this.processVar)
    this.requiredNodeIdList = this.form.requiredNodeId.split(',') || []
    this.allowEditNodeIdList = this.form.allowEditNodeId.split(',') || []
    this.hiddenNodeIdList = this.form.hiddenNodeId.split(',') || []
    this.getNodeList()
    ...
    onSubmit() {
      ...
      _this.$refs['form'].validate((valid) => {
        if (valid) {
          ...
          postData.requiredNodeId = this.requiredNodeIdList.join(',') || ''
          postData.allowEditNodeId = this.allowEditNodeIdList.join(',') || ''
          postData.hiddenNodeId = this.hiddenNodeIdList.join(',') || ''
          updateVarDict(postData).then(response => {
